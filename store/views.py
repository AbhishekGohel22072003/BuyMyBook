...
from .models import Category, Writer, Book, Review
...
def get_book_category(request, id):
    category = get_object_or_404(Category, id=id)  # Retrieve the Category object
    book_ = Book.objects.filter(category_id=id)
    paginator = Paginator(book_, 10)
    page = request.GET.get('page')
    book = paginator.get_page(page)
    context = {
        "category": category,  # Add the category to the context
        "book": book
    }
